...
class PHPEP {
	...
	private function gobbleToken() {
		...
		} elseif ( $ch === self::SQUOTE_CODE || $ch === self::DQUOTE_CODE ) {
			// Single or double quotes
			return $this->gobbleStringLiteral();
		} elseif ( $this->isIdentifierStart( $ch ) || $ch === self::OPAREN_CODE ) { // open parenthesis
			// `foo`, `bar.baz`
			return $this->gobbleVariable();
		} elseif ( $ch === self::OBRACK_CODE ) {
	...
	private function gobbleNumericLiteral() {
		...
		} elseif ( $chCode === self::PERIOD_CODE ) {
