...
class TriggerHappy_Auth {
	...
	protected function auth_endpoint( $route ) {
		...
		try {
			...
			if ( 'login' == $route && ! is_user_logged_in() ) {
				TH::get_template( 'auth/form-login.php', [
				...
				TH::get_template( 'auth/form-grant-access.php', [
					'app_name'    => $_REQUEST['app_name'],
					'return_url'  => add_query_arg( [
						'success' => 0,
						'user_id' => sanitize_text_field( $_REQUEST['user_id'] ),
			...
			wp_die( sprintf( __( 'Error: %s.', 'trigger-happy' ), $e->getMessage() ), __( 'Access denied', 'trigger-happy' ), [ 'response' => 401 ] );
		}
	}
	...
	private function maybe_delete_key( $key ) {
		global $wpdb;
		if ( isset( $key['key_id'] ) ) {
			$wpdb->delete( $wpdb->prefix . 'triggerhappy_api_keys', [ 'key_id' => $key['key_id'] ], [ '%d' ] );
		}
	}
}
