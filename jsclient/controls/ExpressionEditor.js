...
import {buildExpression, parseExpression,stringifyExpression, getTypeFromExpression} from '../lib/util';
...
const _ = require ('lodash');
 ...
 class ExpressionEditor extends React.Component {
    ...
    handleClickOutside() {
       this.setState({quickSearch: false, showOperations:false})
   }
	...
	componentWillReceiveProps(props) {
        ...
        if (this.codeMirror) {
    		...
    		cm.setCursor({line: cursorLine , ch : cursorCh},null, { scroll: false  });
    	}
    }
	getExpression() {
	...
	createTagNode(char) {
	   ...
	   React.render(
        ...
        {nodeTag.nodeId && <span  className="node-expression-data-tag__node">
         #{nodeTag.nodeId}
         </span>}
         ...
         {parts.map(p=> {
             ...
             if (result.indexOf('(') == -1) {
                ...
                result = result.replace( /_([a-z])/g, function(r,m) { return " " + m.toUpperCase(); } );
            }
	...
	replaceExpressionNodeTag({nodeId, pinId, nodeLabel, pinLabel, icon, fieldType}, isString) {
		...
		if (!tag) {
			...
			tags[tagId] = { id: 0xe000 + currentLength, nodeId: nodeId, pinId: pinId, nodeLabel: nodeLabel, pinLabel: pinLabel, icon: icon, fieldType:fieldType };
			...
			tags[tagId] = { id: tag, nodeId: nodeId, pinId: pinId, nodeLabel: nodeLabel, pinLabel: pinLabel, icon: icon,fieldType:fieldType, operationValues: tags[tagId].operationValues };
			charMap[String.fromCharCode(tagId)] = tagId;
    ...
    toggleOperations(e,tagId) {
        e.stopPropagation();
        this.setState({operationTagId: tagId, operationX: e.target.offsetLeft, operationY: e.target.offsetTop, operationFieldType: this.nodeTags[tagId].fieldType, showOperations: !this.state.showOperations });
	}
	updateSelectedOperation(v) {
		this.setState({ selectedOperation: v });
	...
	updateExpressionWithOperation(tagId, operationName, operationValues) {
        ...
        if (operationName) {
    		expr = {
                type: 'CallExpression',
                callee: { type:'MemberExpression',object: expr, property: { type: "Identifier", name: operationName }},
                arguments: args
    	  	};
        }
        ...
        if (matches)
        this.setState({ text: this.props.getNodeLabel(matches[1]) + " - " + matches[2], id: dataa });
		//if ( this.props.onChange) {
	...
	render() {
		...
		return (
            ...
            <div className={"node-control-container " + (this.props.errorMessage && 'node-error' || '')}>
           ...
           {allowExpressions && <a href="javascript:void(0)" className="insert-button" onClick={(e)=>this.toggleQuickSearch(e)}>
           ...
           canUseField={(field, node)=>{
               ...
               return !field.visibleTo || field.visibleTo.indexOf(this.props.name) >= 0 && node.nid == this.props.nodeId;
           }}
           />
           {this.state.showOperations && <OperationsPanel
...
const mapStateToProps = (state, ownProps) => {
	...
	for (var i in ownProps.availableFields) {
     ...
     return {
         ...
         getNodeLabel:  (nid)=>nid == 0 ? "Global" : (state.nodes[nid].name),
         getNodeIcon: (nid)=>{
