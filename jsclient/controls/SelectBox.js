...
import { loadDataType } from '../actions'
 ...
 class SelectBox extends React.Component {
	constructor(props) {
		super(props);
        this.state = { text: props.value && props.value.text || '', id:props.value && props.value.id || null  };
	...
	componentWillReceiveProps(props) {
		if (props.value) {
            ...
            if (typeof props.value === "string" || typeof props.value === "number" || typeof props.value === "boolean") {
                ...
                this.setState({  id:props.value });
            } else {
	            this.setState({ text: props.value && props.value.text || props.value && this.lookupValue(props.value.id), id:  props.value && props.value.id  });
    		}
...
ensureSelected() {
    ...
    let avail = props.dataTypeChoices.filter(n=>this.state.quickSearchFilter == null || n.text.indexOf(this.state.quickSearchFilter) >= 0);
    if (avail.length > 0) {
	...
	render() {
        ...
        let text = this.state.text || this.state.id !== null && this.state.id !== "" && choices && choices.filter(r=>r.id == this.state.id).length > 0 && choices.filter(r=>r.id == this.state.id)[0].text || '';

		return (
            <div>
            <div className="node-control-container">
			<div ref={(el) => { this.element = el; }}  className="node-editable-setting">
                <input type="text" value={text} className="node-editable-text" onBlur={(v)=>this.ensureSelected()} onChange={(v)=>this.updateValue(v)} />
			</div>
		   ...
		   {this.props.allowSearch && <div className="node-quick-search__search">
		   <input type="text" className="node-quick-search__search-input" placeholder="search..." value={this.state.quickSearchText} onChange={(e)=>{
