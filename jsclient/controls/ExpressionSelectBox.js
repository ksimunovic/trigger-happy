...
import { loadDataType, setFieldType } from '../actions'
...
class ExpressionSelectBox extends React.Component {
	constructor(props) {
		...
		if (typeof props.value == "string") {
			...
			if (matches)
			this.state = ({ text: props.getNodeLabel(matches[1]) + " - " + matches[2], id: props.value, expr: parseExpression(props.value) });
			else {
				this.state = { id: props.value, expr: parseExpression(props.value)};
			}

		} else {
			this.state = ({ text: props.value.text, id:props.value.id  });
	...
	componentWillReceiveProps(props) {
		...
		if (props.value) {
			if (typeof props.value == "string") {
				let matches = props.value.match(/\{\{_N([0-9]*).([^\}]*)\}\}/i);
				if (matches)
				this.setState({ text: props.getNodeLabel(matches[1]) + " - " + matches[2], id: props.value, expr: parseExpression(props.value) });
			} else {
    ...
    insertField(node,field, subprop, subtype) {
			...
			this.setState({fieldType: subtype || field.type });

        this.setState({ text:expression, id: "{{_N"+node.nid + "." + expression + "}}", expr: parseExpression("{{_N"+node.nid + "." + expression + "}}")   });
...
ensureSelected() {
	...
	setOperationValue(key,value) {
		...
		callee: { type:'MemberExpression',object: expr, property: { type: "Identifier", name: this.state.selectedOperation }},
		 arguments: args
	  	}
		...
		this.setState({operationValues, text: this.props.getNodeLabel(matches[1]) + " - " + matches[2], id: dataa });
		if ( this.props.onChange) {
	...
	render() {
		...
		return (
			...
			{operations && <a href="javascript:void(0)" className="insert-button" onClick={()=>this.toggleOperations()}>
		   ...
		   {this.state.showOperations && <div className="triggerhappy-operations">
		   ...
		   {this.state.selectedOperation && this.state.selectedOperation != null && (
			   <div>
			   <hr />
...
const mapStateToProps = (state, ownProps) => {
	...
	for (var i in ownProps.availableFields) {
     ...
     return {
         ...
         schemas, datatypes: state.datatypes
    };
};
