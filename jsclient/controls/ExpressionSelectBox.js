...
class ExpressionSelectBox extends React.Component {
    ...
    render() {
        ...
        return (
            <div>
            <div className={'node-control-container node-control-expr-select ' + ( this.props.errorMessage && 'node-error' || '' )}>	<div ref={( el ) => {
 this.element = el;
}}  className="node-editable-setting">
                <input type="text" readOnly={this.props.lookupOnly} value={this.state.text} className="node-editable-text" onBlur={( v )=>this.ensureSelected()} onChange={( v )=>this.updateValue( v )} />	</div>	{operations && <a href="javascript:void(0)" className="insert-button" onClick={()=>this.toggleOperations()}>	 <i className="fa fa-cogs"></i>	 </a>}
           ...
           </div>	   {this.state.showOperations && <div className="triggerhappy-operations">	   <label>Operation Type</label>	   <select onChange={e=>this.updateSelectedOperation( e.target.value )}>	<option value="">None</option>	{Object.keys( fieldTypeMethods ).map( m=><option value={m}>{fieldTypeMethods[m].description}</option> )}	   </select>	   {this.state.selectedOperation && null != this.state.selectedOperation && (	   <div>	   <hr />	   {Object.keys( fieldTypeMethods[this.state.selectedOperation].fields ).map( f=>(	   <div>	   <label>{f}</label>	<input type="text" onChange={( e )=>this.setOperationValue( f, e.target.value )} value={this.state.operationValues && this.state.operationValues[f] || ''} />	<div className="desc">{fieldTypeMethods[this.state.selectedOperation].fields[f].description}</div>	   </div>	   ) )}	   </div>	   )}	<a className="button button-small">OK</a>	</div>}	   <QuickSearch	   allowTypes={this.props.type}	   allowExpressions={allowExpressions}	   className="node-quick-search"	   insertField={this.insertField.bind( this )}	   resetControlTypeClicked={this.props.resetControlTypeClicked}	   show={this.state.quickSearch}	   availableFields={this.props.availableFields}	   schemas={this.props.schemas}	   getNodeIcon={this.props.getNodeIcon}	   />
