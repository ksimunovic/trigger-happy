...
class RichEditor extends React.Component {
	constructor(props) {
		...
		this.nodeTags= {};
		this.charMap= {};
		this.state = { value: props.value, operationFieldType: '' };
	}
	createTagNode(nodeId,path,type) {
		...
		/*
			   React.render(
		        ...
		        <span  className="node-expression-data-tag__node">
		         #{nodeTag.nodeId}</span>
		        ...
		        <span className='node-expression-data-tag__prop'>{finalResult}</span>
		         ...
		         <span className="settings-button" onClick={(e)=>this.toggleOperations(e,tagId)}><i className="fa fa-cogs"></i></span></span>,
				 node
			   );
			   */
	   let icon = this.props.getNodeIcon(nodeId);
		...
		expr += "</span>";
        // if (this.props.schemas[type] && this.props.schemas[type].methods)
		 	//expr += '<span class="settings-button"><i class="fa fa-cogs"></i></span>';
		this.nodeTags[nodeId + "." + path] = {html:expr, nodeId:nodeId, path: path, type: type};
		return expr;
	}
				...
				setup: function (editor) {
					...
					self.editor.mountOperationsPanel = function(selectedExpressionElement,mountElement) {
						...
						let renderOp = function() {
							...
							if (fieldTypeMethods)
							ReactDOM.render(
								  <Provider store={window.nodeStore}>
								<OperationsPanel
					...
					editor.settings.content_css += ",http://localhost/wp-content/plugins/trigger-happy/build/expression.css";

				    editor.addButton('insertExpr', {
				      text: '',
				      icon: 'crosshairs',
				      onclick: function () {
				        self.setState({quickSearch : !self.state.quickSearch, quickSearchText: "" })
				      }
				    });

							editor.on( 'beforesetcontent', function( event ) {

								event.content = self.replaceSpecialCharsWithTags(event.content,true);
							});
	...
	render() {
		...
		return (
			<div ref={(el) => { this.element = el; }}  className={"node-editable-setting " + (this.props.errorMessage && 'node-error' || '')}>
...
const mapStateToProps = (state, ownProps) => {
	...
	schemas['string'] = { methods: { "toUpperCase": {
