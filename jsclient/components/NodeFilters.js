...
import {setExpression,setFilters} from '../actions';
...
class NodeSettings extends React.Component {
	constructor(props) {
		...
		this.state = { filterGroups: props.filters || [[""]]};
	}
	componentWillReceiveProps(props) {
		if (props.filters) {
			this.setState({filterGroups: props.filters });
		}
	}
	getCaption() {
		...
		return (<div>
			...
			{this.state.filterGroups.map((group,i)=>(
				<div className="node-filter-group">
					...
					{group.map((row,i2) =>(
						<div>
						{i2 > 0 && <span className='node-filter-label--and'>AND</span>}
						{i2 == 0 && i > 0 && <span className='node-filter-label-or'>OR</span>}

						<NodeFilter filter={row} availableFields={this.props.availableFields}
						onChange={(data)=>this.setFilter(i,i2,data)}
			            fetchNode={this.props.fetchNode} filter={this.state.filterGroups[i][i2]} />
						</div>))}
...
const mapStateToProps = (state, ownProps) => {
    ...
    availableFields.push({nid:'Global',name:'',name2:'',plugin:'',fields:globalFields,type:'start'});
     ...
     return {
		...
		nodeIndex: nodeIndex,
		selectedNode: node,
        settings: (node.fields||[]).filter(n=>n.type != 'flow' && n.dir == 'start'),
		value: {}
    };
};
...
const mapDispatchToProps = dispatch => {
  return {
	  ...
	  setFilters: (nodeId,filters) => { dispatch(setFilters(nodeId,filters)); }
  }
}
