...
import {selectTrigger,navigate, deleteNode, loadDataType} from '../actions';
...
updateInputValue(index,value) {
    ...
    nodeContent = (<div className="node-content">
      <NodeConnectorList fetchNode={this.props.fetchNode} node={this.props.node} type="output" isConnected={(pinIndex)=>this.props.isConnected(this.props.nid, pinIndex,"output")} setPinRef={(i,r)=>this.props.setPinRef(r,this.props.nid,i,'output')} items={this.props.outputs||[]} onStartConnector={(index)=>this.onStartConnector(index)} />
    </div>);
    nodeContent = '';

    let icon = this.state.canCollapse ? (<i className={"collapse-icon fa  fa-chevron-circle-" + (this.state.collapsed ? 'down': 'up')}
    onClick={this.toggleCollapsed.bind(this)} ></i>) : "";
    if (!this.state.canCollapse) {
    ...
    let pluginIcon = this.props.getNodeIcon(this.props.nid);
        ...
        {pluginIcon && <div className={"node-icon node-icon--" + this.props.node.cat}>
            <img src="pluginIcon" />
            </div>
        }
        {!pluginIcon && this.props.node.nodeType == "trigger" && <div className={"node-icon node-icon--" + this.props.node.nodeType}>
            <i className="fa fa-bolt"></i>
            </div>
        }
        {!pluginIcon && this.props.node.nodeType == "action" && <div className={"node-icon node-icon--" + this.props.node.nodeType}>
            <i className="fa fa-tasks"></i>
            </div>
        }

        {!pluginIcon && this.props.node.nodeType == "condition" && <div className={"node-icon node-icon--" + this.props.node.nodeType}>
            ...
            <header className="node-header">
              ...
              {this.props.node.description}</span>
              ) : ""}
              ...
              <hr />
              { this.props.node.nodeType !== 'trigger' ?
