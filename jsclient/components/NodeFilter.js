...
import { connect } from 'react-redux';
 class NodeFilter extends React.Component {
    constructor(props) {
        super(props);
        this.state = { value: {}, fieldType: 'string' };
        if (props.filter) {
            ...
            if (props.filter.left) {
                ...
                this.state = Object.assign({},this.state,{fieldType: fieldType});
    ...
    selectLeftValue(n,v,e,t, text) {
        ...
        let newState = { left: {expr: e, display: text } };

        if (type == 'boolean') {
            newState['right'] = {expr: true, display: "Yes" };
            newState['op'] = 'equals';
        }
    ...
    selectRightValue(n,v,e,t, text) {
        ...
        this.setState({ right: {expr: e, display: text } });
        this.updateFilter({ right: {expr: e, display: text } });
    }
    selectOperator(v) {
        this.setState({ op: v });
        this.updateFilter({ op: v });
    }
    updateFilter(filterState) {

        let state = {right:this.state.right, left: this.state.left, op:this.state.op};
    ...
    getLeftValueForSelect() {
        if (this.state.left) {
            ...
            return { id: this.state.left.expr, text: this.state.left.display };
        }
        return {};
    }
        ...
        getRightValueForSelect() {
            if (this.state.right) {
                ...
                return { id: this.state.right.expr, text: this.state.right.display };
            }
            return {};
        }
    ...
    render() {
        ...
        let isBool = this.state.fieldType == 'boolean';
