...
import {buildExpression} from '../lib/util';
import {ExpressionEditor, SelectBox, ExpressionSelectBox, RichEditor} from '../controls';
 ...
 class NodeInputListItem extends React.Component {
  ...
  onMouseOver() {
	...
	updateValue(index,e) {
		...
		//this.props.onInputValueUpdated(this.props.index, valState);
	}
    componentDidMount() {
	...
	renderInputs() {
		...
		for (var v in values ) {
			let value = values[v];
			inputs.push(<div><input value={value} onChange={this.updateValue.bind(this,v)} type='text' style={{fontSize:10,width:'100%'}} />
			<i onClick={this.addValueOption.bind(this)} className="fa fa-plus flow-add-array-element-icon" /></div>);
		}
		return inputs;
	}
    ...
    getType() {
        ...
        if (this.props.itemType.indexOf('$') == 0) {
            debugger;
    ...
    useControlType(type) {
        this.setState({ controlTypeOverride: type,controlTypeOverrideText: type == null ? null : "Use lookup value" });
    }

	render() {

		let {name} = this.props.item;
		if (this.props.item.type == "flow")
			name = '';
		...
		return (
			<li>
				...
				className={"node-connector node-connector--" + (this.props.item.type || 'basic')}>
...
;
	...
	var availableFields = Object.values(state.nodes).filter((el,i)=>{
        let def = state.definitions[el.type];

        return i<ownProps.nodeIndex ||(def.fields && def.fields.filter(r=>r.dir == 'start').length) || true;
    }).map((n,i)=>{

		let def = state.definitions[n.type];
        var allFields = [];
        if (def && def.fields) {
            allFields = (def.fields && def.fields.filter(f=>f.type !== 'flow' && (f.dir == 'out' || f.dir == 'start')));
        }
    ...
    availableFields.push({nid:'',name:'',name2:'',plugin:'',fields:globalFields,type:'start'});
